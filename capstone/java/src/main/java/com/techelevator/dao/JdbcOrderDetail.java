...
public class JdbcOrderDetail implements OrderDetailDao{
    ...
    @Override
    public OrderDetail createOrderDetails(int orderId, int cakeId, int cakeQuantity) {
        String sql = "INSERT INTO orderdetails(order_id, cake_id, cake_quantity) VALUES (?, ?, ?);";

        try {
            SqlRowSet detail = jdbcTemplate.queryForRowSet(sql, orderId, cakeId, cakeQuantity);
